...
const BaseButton = ({
  ...
  useEffect(() => {
    if (dropdownRef.current && buttonContainerRef.current) {
      ...
      const totalWidth = Array.from(buttons).reduce((total, button) => total + button.getBoundingClientRect().width, 0);
  ...
  return (
    <div className="inline-block relative" ref={buttonContainerRef}>
      ...
      {splitMode && showDropdown && (
        <ul
          className={`absolute z-10 bg-white border border-gray-200 rounded shadow-lg mt-1 overflow-y-auto max-h-40 ${dropdownPosition === 'top' ? 'bottom-full mb-1' : 'top-full'}
